/**
 * Week View Template with Multi-ICS Support
 * Demonstrates multiple calendar source handling with customizable prefixes
 */
...
const CONFIG = {
  ...
  // ========================================
  sourceMapping: {
    // Map sourceName to prefix/emoji that will be prepended to event titles
    // If sourceName is not found, falls back to source index (e.g., "[1] ")
    'Work': '💼 ',
    'Personal': '🏠 ',
    'Family': '👨‍👩‍👧 ',
    'Sports': '⚽ ',
  ...
  // ========================================
  layout: {
    dayCellWidth: '120px',              // Width of the day column (left side)
    dayCellHeight: '120px',             // Minimum height of each day row
    dayCellPaddingTop: '10px',          // Top padding inside day cell
    dayCellPaddingBottom: '10px',       // Bottom padding inside day cell
    tableMargin: '10px',                // Margin around the main week table
    cellPadding: '5px'                  // Padding inside event cells
  },
  ...
  fontStyle: {
    dayName: {
      size: '65px',                     // Font size for day name (mon, tue, etc.)
    ...
    dayNumber: {
      size: '25px',                     // Font size for day number (1, 2, 3, etc.)
    ...
    weatherIcon: {
      size: '35px',                     // Size of weather icon (SVG)
      color: '#555555'                  // Color of weather icon
    },
    weatherTemp: {
      size: '25px',                     // Font size for min/max temperature
      family: 'Verdana, sans-serif',
      weight: 'normal',
      style: 'normal',
      color: '#555555'
    ...
    // ========================================
    eventTitle: {
      size: '28px',                     // Font size for event title
      family: 'Verdana, sans-serif',
      weight: 'normal',
      style: 'normal',
      color: '#000000',
    ...
    eventTime: {
      size: '26px',                     // Font size for event time
      family: 'Verdana, sans-serif',
      weight: 'normal',
      style: 'normal',
      color: '#797979ff'
    ...
    noEvents: {
      size: '30px',                     // Font size for "no events" message
      family: 'Verdana, sans-serif',
      weight: 'normal',
      style: 'normal',
      color: '#797979ff'
  ...
  todayWeather: {
    ...
    fontSize: '26px',                   // Font size for time/temperature
    iconSize: '60px',                   // Size of weather icons
    rowSpacing: '0px',                  // Vertical padding between rows
    timeRowTopPadding: '25px',          // Extra top padding for time row
    tempRowBottomPadding: '5px',        // Extra bottom padding for temperature row
    labelWidth: '50px',                 // Width of the "today" label column
    
    // Time window configuration
    startHour: 8,                       // Start hour (0-23, must be full hour)
    endHour: 24,                        // End hour (1-24, must be full hour, exclusive)
    numberOfIntervals: 6                // Number of time blocks to display (divides time window evenly)
  },
...
module.exports = function weekView(data) {
  ...
  // ========================================
  // Multi-ICS Helper Functions
  // ========================================
  
  /**
   * Get the prefix for an event based on its source
   * @param {Object} event - The event object with source/sourceName fields
   * @returns {string} - The prefix to prepend to the event title
   */
  const getEventPrefix = (event) => {
    // If the event has a sourceName and we have a mapping for it, use it
    // Use hasOwnProperty to allow empty string ('') as a valid prefix
    if (event.sourceName && CONFIG.sourceMapping.hasOwnProperty(event.sourceName)) {
      return CONFIG.sourceMapping[event.sourceName];
    }
    
    // If the event has a source index but no sourceName, use index as fallback
    if (typeof event.source === 'number' && event.source > 0) {
      return `[${event.source}] `;
    }
    
    // No prefix for single-source calendars or source 0
    return '';
  };
  ...
  // Helper: Get WMO Weather code icon (SVG path from Material Design Icons)
  ...
  // Helper: Find most common value in array (statistical mode)
  const findMode = (arr) => {
    const frequency = {};
    arr.forEach(val => {
  ...
  // Helper: Process Open-Meteo API data into daily weather
  ...
  // Helper: Create today's hourly weather table
  ...
  // Process weather data from extra (supports both formats)
  let weatherData = {};
  if (extra.hourly) {
    // Open-Meteo format
  ...
  events.forEach(event => {
    ...
    // For all-day events, work with date-only comparisons
    if (event.allDay) {
  ...
  days.forEach(day => {
    ...
    const dayWeather = weatherData[dateKey] || { icon: 'M12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,2L14.39,5.42C13.65,5.15 12.84,5 12,5C11.16,5 10.35,5.15 9.61,5.42L12,2M3.34,7L7.5,6.65C6.9,7.16 6.36,7.78 5.94,8.5C5.5,9.24 5.25,10 5.11,10.79L3.34,7M3.36,17L5.12,13.23C5.26,14 5.53,14.78 5.95,15.5C6.37,16.24 6.91,16.86 7.5,17.37L3.36,17M20.65,7L18.88,10.79C18.74,10 18.47,9.23 18.05,8.5C17.63,7.78 17.1,7.15 16.5,6.64L20.65,7M20.64,17L16.5,17.36C17.09,16.85 17.62,16.22 18.04,15.5C18.46,14.77 18.73,14 18.87,13.21L20.64,17M12,22L9.59,18.56C10.33,18.83 11.14,19 12,19C12.82,19 13.63,18.83 14.37,18.56L12,22Z', minTemp: 18, maxTemp: 22 };
    const weatherIcon = dayWeather.icon || dayWeather.emoji;
    const weatherMinTemp = typeof dayWeather.minTemp === 'number' ? `${dayWeather.minTemp}°` : (dayWeather.temp ? `${dayWeather.temp}°` : '18°');
    const weatherMaxTemp = typeof dayWeather.maxTemp === 'number' ? `${dayWeather.maxTemp}°` : (dayWeather.temp ? `${dayWeather.temp}°` : '22°');
    
    // Build inner event table
    let eventTable = '';
    if (dayEvents.length === 0) {
      ...
      dayEvents.forEach(event => {
        // Apply prefix based on source mapping
