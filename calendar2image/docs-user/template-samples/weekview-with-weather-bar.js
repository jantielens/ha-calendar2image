...
module.exports = function weekView(data) {
  ...
  events.forEach(event => {
    ...
    // All-day events may have timezone offsets (e.g., 01:00:00Z for Europe/Brussels)
    // so we need to compare only the calendar dates
    if (event.allDay) {
  ...
  days.forEach(day => {
    ...
    // Get weather data for this day (use ISO date string as key: YYYY-MM-DD)
    ...
    const weatherIcon = dayWeather.icon || dayWeather.emoji; // Support both formats
